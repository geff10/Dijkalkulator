...
public class JDialogPointTypes extends JDialog {
	...
	String defaultConfigFile = 
			System.getProperty("user.dir") + System.getProperty("file.separator")+ "point.config";
	String lastConfigFile = "";
	String defaultTitle = "Pontok";
	...
	public JDialogPointTypes() {
		...
		tableModelPT = new DefaultTableModel(data, new String[] { "Pont kódja", "Pontcsoport" });
		tablePointTypes_1.setModel(tableModelPT);
		tableModelPT.addTableModelListener(new TableModelListener() {
			public void tableChanged(TableModelEvent tme) {
				if (tme.getType() == TableModelEvent.UPDATE) { // something changed
	...
	private void saveSimply(String file2Save) {
		if (tableModelPT.getRowCount() == 0) //nothing to save
			return;
